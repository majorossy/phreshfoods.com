...
async function checkSiteStatus() {
  try {
    ...
    const apiResponse = await fetch('http://localhost:3000/api/farm-stands');
    const apiData = await apiResponse.json();

    console.log('✅ Site Status Check:');
    console.log('  - Frontend: ' + (frontendHtml.includes('PhreshFoods') ? '✓ Running' : '✗ Error'));
    console.log('  - Backend API: ' + (apiData.farmStands ? `✓ Serving ${apiData.farmStands.length} locations` : '✗ Error'));
