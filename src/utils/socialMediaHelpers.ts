...
/**
 * Constructs X profile URL from shop data
 */
export function getXUrl(shop: Shop): string | null {
  if (!shop.XHandle) {
    return null;
  }

  const handle = shop.XHandle.trim().replace('@', '');

  return `https://x.com/${handle}`;
}

/**
 * Gets display name for X (handle with @)
 */
export function getXDisplayName(shop: Shop): string | null {
  if (!shop.XHandle) {
    return null;
  }

  const handle = shop.XHandle.trim();
  return handle.startsWith('@') ? handle : `@${handle}`;
}

/**
 * Generates X/Twitter timeline embed HTML
 * Uses the old Twitter timeline widget with twitter.com URLs
 * Note: X has deprecated timeline embeds, but this may still work
 */
export function getXTimelineEmbed(shop: Shop): string | null {
  const handle = shop.XHandle?.trim().replace('@', '');
  ...
  const displayName = shop.placeDetails?.name || shop.Name;

  // Using twitter.com URL instead of x.com for the old embed widget
  return `
...
export function getSocialMediaCount(shop: Shop): number {
  ...
  if (shop.XHandle) count++;
  return count;
}

/**
 * Generates Instagram profile embed HTML code
 * Returns the embed blockquote HTML that Instagram's embed.js will process
 */
export function getInstagramProfileEmbed(shop: Shop): string | null {
  const username = getInstagramDisplayName(shop);
  ...
  return `
<blockquote class="instagram-media" data-instgrm-permalink="${embedUrl}" data-instgrm-version="14" style="background:#FFF; border:0; border-radius:3px; box-shadow:0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15); margin: 1px; max-width:540px; min-width:326px; padding:0; width:99.375%; width:-webkit-calc(100% - 2px); width:calc(100% - 2px);">
  <div style="padding:16px;">
    ...
    <p style="color:#c9c8cd; font-family:Arial,sans-serif; font-size:14px; line-height:17px; margin-bottom:0; margin-top:8px; overflow:hidden; padding:8px 0 7px; text-align:center; text-overflow:ellipsis; white-space:nowrap;">
      <a href="${embedUrl}" style="color:#c9c8cd; font-family:Arial,sans-serif; font-size:14px; font-style:normal; font-weight:normal; line-height:17px;" target="_blank">${displayName}</a> (@<a href="${embedUrl}" style="color:#c9c8cd; font-family:Arial,sans-serif; font-size:14px; font-style:normal; font-weight:normal; line-height:17px;" target="_blank">${username}</a>) • Instagram photos and videos
    </p>
  </div>
</blockquote>
  `.trim();
