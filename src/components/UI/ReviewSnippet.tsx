...
const ReviewSnippet: React.FC<ReviewSnippetProps> = ({
  ...
  return (
    <div
      id={`review-snippet-${shop.slug || shop.GoogleProfileID}`}
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <p id={`review-text-${shop.slug || shop.GoogleProfileID}`} className="text-gray-600 dark:text-gray-400 italic line-clamp-2">
            {truncatedText}
          </p>

          {/* Author info */}
          <div id={`review-author-info-${shop.slug || shop.GoogleProfileID}`} className="flex items-center gap-1.5 mt-1">
            ...
            {/* Author name */}
            <span id={`review-author-name-${shop.slug || shop.GoogleProfileID}`} className="text-gray-500 dark:text-gray-500 font-medium">
              {shortAuthor}
            </span>

            {/* Rating stars (small) */}
            {review.rating > 0 && (
              <span id={`review-rating-${shop.slug || shop.GoogleProfileID}`} className="text-yellow-500 flex items-center">
                <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <span id={`review-rating-value-${shop.slug || shop.GoogleProfileID}`} className="text-[10px] ml-0.5">{review.rating}</span>
              </span>
            )}
          </div>
        </div>
